...
let create todos storage =
  let calculate_totals rl =
    ...
    let remaining =
      todos |> List.filter (Todo.complete >> not) |> List.length
    in
...
let info_footer =
  footer
    ...
    [
      ...
      p
        [
          txt "Part of "; a ~a:[ a_href "http://todomvc.com" ] [ txt "TodoMVC" ];
        ];
    ]
  |> To_dom.of_footer
...
let main default_todos (_ : #Dom_html.event Js.t) =
  ...
  let todo_app =
    section
      ~a:[ a_class [ "todoapp" ] ]
      [ New_todo.render ~dispatch; main_section t ]
    |> To_dom.of_section
  in
...
let () =
  let todos =
    ...
    |> List.map (fun (complete, description) ->
           Todo.create ~complete description)
  in
